...
public class OrderRepositoryAdapter implements OrderRepository {
    ...
    public OrderRecord save(OrderRecord order) {
        ...
        return new OrderRecord(saved.id, saved.productId, saved.quantity, saved.total);
    }

    @Override
    public Optional<OrderRecord> findById(Long id) {
        return repository.findById(id)
                .map(entity -> new OrderRecord(entity.id, entity.productId, entity.quantity, entity.total));
    }
}
